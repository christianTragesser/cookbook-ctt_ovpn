...
bash 'configure openvpn container' do
  user 'root'
  code <<-EOF
  /bin/expect -c 'spawn docker run --volumes-from #{node[:ctt_ovpn][:data_container]} --rm -it kylemanna/openvpn ovpn_initpki nopass; expect "RSA CA.:"; send "test\r"; expect eof'
  sleep 45
  EOF
